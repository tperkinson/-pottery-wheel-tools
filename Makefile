...
THROWING_WATER_HOLDER_SCAD := attachments/throwing_water_holder/v1/throwing_water_holder_v1.scad
...
CHECK_SCADS := $(CORE_SWEEP_SCAD) $(PHONE_SCAD) $(TRAY_SCAD) $(CUP_HOLDER_SCAD) $(THROWING_WATER_HOLDER_SCAD) $(RADIAL_TOOL_HOLDER_SCAD) $(STAMP_TLP_SCAD)
...
THROWING_WATER_HOLDER_STL := $(BUILD_DIR)/throwing_water_holder_v1.stl
THROWING_WATER_HOLDER_SAMPLE_STL := $(BUILD_DIR)/throwing_water_holder_v1_holder_sample.stl
...
.PHONY: help all core attachments stamps phone tray cup_holder throwing_water_holder radial_tool_holder check clean
...
attachments: phone tray cup_holder throwing_water_holder radial_tool_holder
...
check: | $(CHECK_DIR)
	...
	echo "CHECK attachments/throwing_water_holder/v1/throwing_water_holder_v1.scad (holder-only mode)"; \
	$(OPENSCAD) -D '$$fn=24' -D render_holder_only=true -o "$(CHECK_DIR)/throwing_water_holder_v1_holder_sample.stl" "$(THROWING_WATER_HOLDER_SCAD)"; \
