...
// Sturdy radial drape bar with a lighter shared-wall web, dual-sided gussets,
// and holder-only print orientations using the canonical C-clamp sweep.
...
holder_only_print_mode = "as_modeled"; // "as_modeled", "side", "bar_vertical"
...
web_embed_depth = 6;        // mm down into clamp wall (below top)
web_height_above_top = 4;   // mm above clamp top

back_gusset_enable = true;  // add gussets on clamp-side of the web
back_gusset_depth = 4;      // mm into clamp side
back_gusset_height = 12;    // mm (height of back gusset ribs)
...
web_total_height = web_embed_depth + web_height_above_top;
web_z0 = -web_embed_depth;
web_x = wall_local_x - shared_wall_overlap;
back_gusset_x = web_x - back_gusset_depth;
back_gusset_height_eff = min(back_gusset_height, web_total_height);
...
if (web_total_height <= 0)
    echo("WARNING: web_total_height must be > 0.");
if (web_embed_depth < 0)
    echo("WARNING: web_embed_depth must be >= 0.");
if (back_gusset_enable && back_gusset_depth < 0)
    echo("WARNING: back_gusset_depth must be >= 0.");
...
module gusset_plate(y_center) {
    ...
    hull() {
        translate([web_x, y_center - gusset_thickness / 2, 0])
...
module back_gusset_plate(y_center) {
    if (back_gusset_enable && back_gusset_depth > 0 && back_gusset_height_eff > 0) {
        base_h = max(join_eps, min(back_gusset_height_eff, web_total_height));
        hull() {
            translate([web_x - join_eps, y_center - gusset_thickness / 2, web_z0])
                cube([join_eps, gusset_thickness, base_h], center = false);
            translate([back_gusset_x, y_center - gusset_thickness / 2, web_z0])
                cube([back_gusset_depth + join_eps, gusset_thickness, base_h * 0.6], center = false);
        }
    }
}

module gusset_wall_web() {
    translate([web_x, -gusset_span / 2, web_z0])
        cube([gusset_thickness, gusset_span, web_total_height], center = false);
}
...
module holder_linear() {
    union() {
        ...
        if (show_gussets) {
            ...
            for (y = gusset_plate_offsets) back_gusset_plate(y);
            bar_mount_block();
        }
    }
}
...
module holder_only_output() {
    if (holder_only_print_mode == "side") {
        rotate([90, 0, 0])
            holder_body();
    } else if (holder_only_print_mode == "bar_vertical") {
        rotate([0, -90, 0])
            holder_body();
    } else {
        holder_body();
...
if (show_profile) {
    ...
    if (render_holder_only) {
        if (show_holder) holder_only_output();
    } else {
        clamp_pad_sweep();
